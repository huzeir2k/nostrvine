...
void main() {
  group('Video Upload → Publish Flow', () {
    ...
    test('publishing should fail when upload has not completed (videoId is null)', () async {
      ...
      expect(upload.videoId, isNull, reason: 'Upload should not have videoId before completion');
      expect(upload.cdnUrl, isNull, reason: 'Upload should not have cdnUrl before completion');
      ...
      expect(canPublish, isFalse,
          reason: 'Should not be able to publish upload without videoId and cdnUrl');
    });

    test('publishing should succeed when upload has completed (videoId populated)', () async {
      ...
      final canPublish = completedUpload.videoId != null && completedUpload.cdnUrl != null;
      ...
      expect(completedUpload.cdnUrl, equals('https://cdn.example.com/video_123.mp4'));
      expect(completedUpload.status, equals(UploadStatus.readyToPublish));
    });

    test('startUpload should complete async upload before returning for publish', () async {
      ...
      expect(true, isTrue, reason: 'Test placeholder - implementation needed');
    });
  });
}
