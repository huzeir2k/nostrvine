  ...
  group('AUTH and Kind 22 Event Retrieval - Real relay3.openvine.co Relay', () {
    ...
    test('AUTH completion tracking works correctly', () async {
      ...
      final authSubscription = nostrService.authStateStream.listen((states) {
        ...
        Log.debug('AUTH state change: $states',
      ...
      try {
        ...
        for (final entry in relayStatuses.entries) {
          Log.debug('Relay ${entry.key}: ${entry.value}',
              name: 'AuthTest', category: LogCategory.system);
        }
      } finally {
    ...
    test('Kind 22 events can be retrieved from relay3.openvine.co after AUTH',
      ...
      Log.info('Subscribing to Kind 22 video events...',
          name: 'AuthTest', category: LogCategory.system);
      ...
      Log.info('Waiting for Kind 22 events...',
          name: 'AuthTest', category: LogCategory.system);
    ...
    test('AUTH retry mechanism works when AUTH completes late', () async {
      ...
      Log.info('Subscribing before AUTH completion...',
          name: 'AuthTest', category: LogCategory.system);
      await videoEventService.subscribeToVideoFeed(
        subscriptionType: SubscriptionType.discovery,
        limit: 10,
      ...
      if (nostrService.isVineRelayAuthenticated) {
        Log.info('✅ AUTH completed - retry mechanism should have triggered',
            name: 'AuthTest', category: LogCategory.system);
        // We might have received events through retry
      } else {
        Log.warning('⚠️ AUTH still not complete after extended wait',
            name: 'AuthTest', category: LogCategory.system);
      }
    }, timeout: const Timeout(Duration(minutes: 2)));
    ...
    test('AUTH session persistence works across service restarts', () async {
      ...
      try {
        ...
        if (firstAuthState) {
          Log.info('Testing AUTH session persistence...',
              name: 'AuthTest', category: LogCategory.system);
          ...
          Log.debug('AUTH states loaded: ${secondService.relayAuthStates}',
              name: 'AuthTest', category: LogCategory.system);
        }

        Log.info('✅ AUTH session persistence mechanism tested',
            name: 'AuthTest', category: LogCategory.system);
      } finally {
        firstService?.dispose();
    ...
    test('Configurable AUTH timeout works correctly', () async {
      ...
      for (final timeout in timeouts) {
        Log.info('Testing AUTH timeout: ${timeout.inSeconds}s',
            name: 'AuthTest', category: LogCategory.system);
    ...
    test('Multiple relays AUTH state tracking', () async {
      ...
      Log.info('✅ Multiple relay AUTH tracking completed',
          name: 'AuthTest', category: LogCategory.system);
    }, timeout: const Timeout(Duration(minutes: 2)));
  });
}
