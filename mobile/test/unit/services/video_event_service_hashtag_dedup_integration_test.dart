  ...
  group('VideoEventService Hashtag Deduplication Integration Test', () {
    test('Verify deduplication logic works correctly', () {
      ...
      final video1 = VideoEvent(
        id: 'video-1',
        pubkey: 'd0aa74d68e414f0305db9f7dc96ec32e616502e6ccf5bbf5739de19a96b67f3e',
      ...
      final video2 = VideoEvent(
        id: 'video-2',
        pubkey: '70ed6c56d6fb355f102a1e985741b5ee65f6ae9f772e028894b321bc74854082',
        createdAt: DateTime.now().millisecondsSinceEpoch ~/ 1000,
        content: 'Cat video',
        timestamp: DateTime.now(),
        hashtags: ['cat', 'cute'],
      ...
      for (final events in eventLists.values) {
        buggyResult.addAll(events.where((event) => 
          ['dog'].any((tag) => event.hashtags.contains(tag))
        ));
      }
      ...
      for (final events in eventLists.values) {
        for (final event in events.where((event) => 
          ['cute'].any((tag) => event.hashtags.contains(tag))
        )) {
      ...
      expect(multiTagResult.map((v) => v.id).toSet(), equals({'video-1', 'video-2'}));
      ...
      for (final events in eventLists.values) {
        for (final event in events.where((event) => 
          ['bird'].any((tag) => event.hashtags.contains(tag))
        )) {
