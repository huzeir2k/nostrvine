...
void main() {
  group('VideoEventsProvider - Fresh First', () {
    ...
    test('emits videos with unseen first on initial load', () async {
      ...
      container1.listen<AsyncValue<List<VideoEvent>>>(
        videoEventsProvider,
        (previous, next) {
          if (next.hasValue) {
      ...
      expect(states.isNotEmpty, true, reason: 'Should have received videos from relay');
      final videos = states.last;
      expect(videos.length, greaterThanOrEqualTo(4), reason: 'Should have at least 4 videos');
      ...
      container2.listen<AsyncValue<List<VideoEvent>>>(
        videoEventsProvider,
        (previous, next) {
          if (next.hasValue) {
      ...
      expect(states2.isNotEmpty, true, reason: 'Should have received videos on restart');
      ...
      print('Last unseen index: ${unseenIndices.isNotEmpty ? unseenIndices.last : "none"}');
      print('First seen index: ${seenIndices.isNotEmpty ? seenIndices.first : "none"}');

      // Verify: All unseen videos should come before all seen videos
      if (unseenIndices.isNotEmpty && seenIndices.isNotEmpty) {
        expect(unseenIndices.last < seenIndices.first, true,
            reason: 'On app restart, all unseen videos should come before all seen videos. Last unseen at ${unseenIndices.last}, first seen at ${seenIndices.first}');
      }
